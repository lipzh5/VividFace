...
from custommodels.modules.liveportrait_utils.timer import Timer
from custommodels.modules.liveportrait_utils.helper import load_model, concat_feat, load_mapping_network
from custommodels.modules.liveportrait_utils.camera import headpose_pred_to_degree, get_rotation_matrix
from custommodels.modules.liveportrait_utils.retargeting_utils import calc_eye_close_ratio, calc_lip_close_ratio
...
class ExpShadowingWrapper(object):
    """
    Wrapper for Human
    """
    ...
    def __init__(self, inference_cfg: InferenceConfig):
        ...
        # init G
        model_type = 'spade_generator_multitask' if inference_cfg.flag_multitask_spade else 'spade_generator'
        self.spade_generator = load_model(inference_cfg.checkpoint_G, model_config, self.device, model_type)
    ...
    def prepare_img_rgb(self, img: np.ndarray) -> torch.Tensor:
        """ construct the input as standard
        img: 3xHxW, uint8, 256x256
        """
        x = img.copy()
        x = x[np.newaxis].astype(np.float32) / 255. # HxWx3 -> 1xHxWx3, normalized to 0-1
        x = np.clip(x, 0, 1)  # clip to 0~1
    ...
    def get_control_values(self, images):
        ...
        # print(f'shape pos pred _vals: {pos_pred_vals.shape}')
        return pos_pred_vals.cpu()

...
class Cropper(object):
	...
	def crop_driving_image(self,frame:bytes) -> np.ndarray:
		nparr = np.frombuffer(frame, np.uint8)
		image = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
		# cv2.imwrite('debug_frame.png', image)
		# cv2.imwrite(f'debug_frame_{time.time()}.png', image)

		h, w = image.shape[:2]
		# image = np.asarray(Image.open(BytesIO(frame)))
		faces = app.get(image)
		if not faces:
		...
		for i, face in enumerate(faces):
			...
			# Make square box based on largest side
			box_size = max(x2 - x1, y2 - y1)

			# Optional: scale box a bit (e.g., 1.2x for margin)
			scale = 2.2
			box_size = int(box_size * scale)  # 150


			# Calculate new square box coordinates
			x1_square = max(0, cx - box_size // 2)
			y1_square = max(0, cy - box_size // 2)
			x2_square = min(w, cx + box_size // 2)
			y2_square = min(h, cy + box_size // 2)

			# Crop face region from original image
			cropped_face = image[y1_square:y2_square, x1_square:x2_square]
			# print(f'x1: {x1}, {y1}, {x2}, {y2}')
			if cropped_face.size == 0:
				print("Skipping empty cropped face")
				continue
			# # Save or display the cropped face
			# filename = f"debug_face/face_{time.time()}.jpg"
			
			# cv2.imwrite(f"face_{time.time()}.jpg", cropped_face)
			# print(f"Saved: {filename}")
			print(f'cropped face shape: {cropped_face.shape}')
			return cv2.resize(cropped_face, (256, 256))

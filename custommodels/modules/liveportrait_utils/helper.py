...
from custommodels.modules.exp_shadowing_modules.mapping_net import X2Control
from custommodels.modules.exp_shadowing_modules.spade_generator import SPADEDecoder
from custommodels.modules.exp_shadowing_modules.warping_net import WarpingNetwork
from custommodels.modules.exp_shadowing_modules.motion_extractor import MotionExtractor
from custommodels.modules.exp_shadowing_modules.appearance_feature_extractor import AppearanceFeatureExtractor
from custommodels.modules.exp_shadowing_modules.stitching_retargeting_net import StitchingRetargetingNetwork
...
def load_mapping_network(ckpt_path, cfg, device):
    ...
    mean, std = cfg.data.imagenet.mean, cfg.data.imagenet.std
...
def load_model(ckpt_path, model_config, device, model_type):
    ...
    # elif model_type == 'spade_generator_multitask':
    #     model = MultitaskSPADEDecoder(**model_params).to(device)
...
def serialize_images(image_folder='/home/penny/pycharmprojects/AmecaBackend/Assets/debug_imgs_michael'):
	images = os.listdir(image_folder)
	images.sort()
	images = [osp.join(image_folder, image) for image in images]
	bytes_list = [serialize_image(image) for image in images]
	return bytes_list
...
def serialize_image(image_path):
	...
	# Save image to buffer in desired format (e.g., JPEG or PNG)
	image.save(buffer, format="JPEG")  # or PNG, etc.

	# Get byte data
	img_bytes = buffer.getvalue()

	# Now img_bytes is a `bytes` object you can send/store
	# print(type(img_bytes))  # <class 'bytes'>
	# print(len(img_bytes))   # Size in bytes
